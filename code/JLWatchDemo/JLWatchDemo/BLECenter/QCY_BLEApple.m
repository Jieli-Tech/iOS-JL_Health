                 ...
                 RSSI:(NSNumber *)RSSI
{
    ...
    // ota升级过程，回连使用
    NSString *bleAddr = [[JL_RunSDK sharedMe] bleAddr];
    if ([JL_BLEAction otaBleMacAddress:bleAddr isEqualToCBAdvDataManufacturerData:ble_AD]) {
        NSLog(@"---> OTA 回连地址:%@",bleAddr);
...
{
    NSLog(@"BLE Connected ---> De vice %@",peripheral.name);
