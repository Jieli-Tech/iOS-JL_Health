...
@interface JLPagerBaseView () <UIScrollViewDelegate, UIGestureRecognizerDelegate>
...
- (void)setTitleArray:(NSArray *)titleArray {
    ...
    if (_titleArray.count == btnArray.count) {
        for (int i = 0; i < _titleArray.count; i++) {
            JLTopTabButton *btn = btnArray[i];
            NSString *titleString = _titleArray[i];
            [btn setTitle:titleString];
        }
    }
    if (![self.subviews containsObject:self.topTab]) {
    ...
    if (!_topTab) {
        ...
        for (NSInteger i = 0; i < _titleArray.count; i++) {
            ...
            if (_titleArray.count > 5) {
                ...
                buttonFrame = CGRectMake(_topTab.frame.size.width / _titleArray.count * i, 0, _topTab.frame.size.width / _titleArray.count, PageBtn);
            }
