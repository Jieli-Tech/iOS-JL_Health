...
-(void)setupUI{
    ...
    [btnManager mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    [imageMore mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(self.mas_right);
        make.height.equalTo(@22);
    ...
    CGFloat itemW = (width-40)/3;//110.0;
    ...
    [subCollectView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(self.mas_right);
...
-(__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    UIImage *phimage = [UIImage imageNamed:@"watch_img_06"];
    if (md.mInfoDict && md.mInfoDict[@"icon"]) {
        [cell.watchDialImageView sd_setImageWithURL:[NSURL URLWithString:md.mInfoDict[@"icon"]] placeholderImage:phimage options:SDWebImageAllowInvalidSSLCertificates];
        
    }else{
        NSData *imgData = [WatchMarket getDataOfWatchIcon:md.mName];
        UIImage *image = [UIImage imageWithData:imgData];
        if (image) {
            [cell.watchDialImageView setImage:image];
        }else{
            //cell.subImageView.image = phimage;
            [cell.watchDialImageView setImage:phimage];
        }
    }
    ...
    [cell.subDeleteBtn setImage:[UIImage imageNamed:@"product_icon_delete_nol"] forState:UIControlStateNormal];
    if (isManager) {
