...
-(void)noteCardArray:(NSNotification *)note{
    ...
    if (![uuid isEqualToString:kJL_BLE_Uuid]) {
        return;
    }
...
-(void)handleFileMonitor{
    ...
    [kJL_BLE_CmdManager.mFileManager cmdBrowseMonitorResult:^(NSArray * _Nullable array, JL_BrowseReason reason) {
        switch (reason) {
            ...
            case JL_BrowseReasonFolderEnd:{
                ...
                if (array.count == 0 && self->reqNum != -1) {
                    if ([self->_delegate respondsToSelector:@selector(dmHandleWithItemModelArray:)]) {
                        ...
                        [self->_delegate dmHandleWithItemModelArray:tempArray];
                    }
                }
                self->reqNum = -1;
...
#pragma mark 过滤非音乐文件
-(NSArray *)filter:(NSArray *)itemArray{
    NSMutableArray  *tempArr = [[NSMutableArray alloc]init];
    for (JLModel_File *modelFile in itemArray) {
        NSString *fileName = [modelFile.fileName lowercaseString];
        if((modelFile.fileType != JL_BrowseTypeFile) || [fileName containsString:@".mp3"]
           || [fileName containsString:@".wav"]
           || [fileName containsString:@".wave"]
           || [fileName containsString:@".aac"]
           || [fileName containsString:@".ogg"]
           || [fileName containsString:@".amr"]
           || [fileName containsString:@".wma"]
           || [fileName containsString:@".flac"]
           || [fileName containsString:@".ape"]){
            [tempArr addObject:modelFile];
        }
    }
    return  [tempArr copy];
}
