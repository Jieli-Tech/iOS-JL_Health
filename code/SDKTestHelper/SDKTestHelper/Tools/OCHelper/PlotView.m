...
- (CGFloat)coordinateYFromShort:(short)s {
    CGFloat f = 0.0;
    f = (s / maxY + 1) * (self.bounds.size.height + 10) / 2;
//    NSLog(@"%d, %.2f", s, f);
    return f;
}
...
- (void)drawRect:(CGRect)rect {
    ...
    NSString *text = [NSString stringWithFormat:@"峰值: %.0f 时长：%.2f s,数据大小：%d Byte", maxY,[self calculateDuration:self.fm.mSampleRate BitDepth:self.fm.mBitsPerChannel Channel:self.fm.mChannelsPerFrame Data:_points],(int)_points.length];
    // 文本段落样式
...
-(NSTimeInterval)calculateDuration:(int)sampleRate BitDepth:(int)bit Channel:(int)channel Data:(NSData *)dt{
    double totalSamples = (double)dt.length/((double)bit/8);
    double dur = totalSamples/(double)(sampleRate*channel);
    return dur;
}
