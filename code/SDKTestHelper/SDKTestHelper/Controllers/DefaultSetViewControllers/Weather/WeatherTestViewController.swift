...
class WeatherTestViewController: BaseViewController, UIPickerViewDelegate ,UIPickerViewDataSource{
    ...
    override func initUI(){
        ...
        navigationView.title = "Weather Test"
        ...
        self.view.addSubview(pickerViewLab)
        ...
        self.view.addSubview(sendPickerView)
    ...
    @objc func didSend(){
        ...
        if selecterView.subSwitch.isOn == true {
            timeSender.invalidate()
            timeSender = Timer.scheduledTimer(timeInterval: TimeInterval(self.sendSec), target: self, selector: #selector(sendAction), userInfo: nil, repeats: true)
            timeSender.fire()
    ...
    @objc func sendAction(){
        ...
        JLWearable.sharedInstance().w_syncWeather(weather, withEntity: BleManager.shared.currentEntity!) { status in
            self.isSendding = false
