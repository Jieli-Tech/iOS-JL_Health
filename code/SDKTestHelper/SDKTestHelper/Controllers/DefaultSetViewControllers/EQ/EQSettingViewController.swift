...
class EQSettingViewController: BaseViewController {
    ...
    @IBOutlet weak var sld_0: UISlider!
    @IBOutlet weak var sld_1: UISlider!
    @IBOutlet weak var sld_2: UISlider!
    @IBOutlet weak var sld_3: UISlider!
    @IBOutlet weak var sld_4: UISlider!
    @IBOutlet weak var sld_5: UISlider!
    @IBOutlet weak var sld_6: UISlider!
    @IBOutlet weak var sld_7: UISlider!
    @IBOutlet weak var sld_8: UISlider!
    @IBOutlet weak var sld_9: UISlider!

    @IBOutlet weak var lb_0: UILabel!
    ...
    override func initUI() {
        super.initUI()
        navigationView.title = "EQ Setting"
    ...
    @IBAction func onSelectFrq(_ sender: UISlider) {
    ...
    func updateEqName( model:JL_EQMode ) -> String {
        if model == .NORMAL  { return "Natural"}
        if model == .ROCK    { return "Rock" }
        if model == .POP     { return "Pop" }
        if model == .CLASSIC { return "Classic" }
        if model == .JAZZ    { return "Jazz" }
        if model == .COUNTRY { return "Country" }
        if model == .CUSTOM  { return "Custom" }
        if model == .LATIN   { return "Latin" }
        if model == .DANCE   { return "Dance" }
        return ""
    }
