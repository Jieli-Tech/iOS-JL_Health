...
class DialTpViewController: BaseViewController, UIImagePickerControllerDelegate & UINavigationControllerDelegate {
    ...
    override func initUI() {
        super.initUI()
        navigationView.title = "dialTransmission"
        navigationView.leftBtn.setTitle("back", for: .normal)
        
        seleteFileBtn.setTitle("selectDial", for: .normal)
        ...
        seleteImageFileBtn.setTitle("selectBackground", for: .normal)
        ...
        transpFileLab.text = "noFileSelected"
        ...
        dialsArray.bind(to: deviceFileListView.rx.items(cellIdentifier: "DialFileCell")) { index, element, cell in
        ...
        deviceFileListView.rx.itemDeleted.subscribe { [weak self] indexPath in
            ...
            DialManager.deleteFile(path) { st, progress in
                if st == .success{
                    strongSelf.view.makeToast("deleteSuccess",position: .center)
    ...
    override func initData() {
        ...
        cancelBtn.rx.tap.subscribe { [weak self]_ in
            BleManager.shared.currentEntity?.mCmdManager.mFileManager.cmdStopBigFileData()
        ...
        deviceFileListView.rx.modelDeleted(String.self).subscribe { [weak self](item) in
            if let _ = BleManager.shared.currentCmdMgr{
                DialManager.deleteFile("/"+item) { op, progress in
                    if op == .success{
                        self?.view.makeToast("deletionOfWatchDialSuccessful"
                                             ,position: .center)
                        self?.requestData()
                    }
                }
    ...
    func handleTransport(){
        ...
        let fileName = "/"+(self.filePath as NSString).lastPathComponent.uppercased()
        ...
        DialManager.addFile(fileName, content: dt) { [weak self](op, progress) in
            ...
            switch op {
            case .noSpace:
                self.view.makeToast("notEnoughSpace",position: .center)
            case .doing:
                self.view.makeToast("transmitting",position: .center)
                self.progress.progress = Float(progress)
                self.progressLab.text = "\(Int(progress * 100))%"
            case .fail:
                self.view.makeToast("transportError",position: .center)
            case .success:
                self.view.makeToast("transportedSuccessfully",position: .center)
                BleManager.shared.currentCmdMgr?.mFlashManager
                    .cmdWatchFlashPath(fileName, flag: .setDial, result: {
                        flash, size, path, describe in
                        if flash != 0 {
                            self.view.makeToast("theDialFileSettingFailedCheckTheReliabilityOfTheDialFile")
                        }
                        self.requestData()
                    })
            case .unnecessary:
                self.view.makeToast("noNeedToTransmit",position: .center)
            case .resetFial:
                self.view.makeToast("resetFailed",position: .center)
            case .normal:
                self.view.makeToast("regular",position: .center)
            case .cmdFail:
                self.view.makeToast("commandExecutionFailed",position: .center)
            @unknown default:
                break
            }
        }
    ...
    func handleTransportWatchBg(){
        let binPath = JL_Tools.listPath(.libraryDirectory,
                                        middlePath: "",
                                        file: self.watchBinName)
        guard let dt = try?Data(contentsOf: URL(fileURLWithPath: binPath)) else{
            return
        }
        DialManager.addFile("/"+self.watchBinName, content: dt) {
    ...
    func showTransportWatchBg(){
        ...
        BleManager.shared.currentCmdMgr?.mFlashManager
            .cmdWatchFlashPath(nil,
                               ...
                               result: { [weak self](flag, size, path, describe) in
                guard let self = self else{return}
                self.watchName = path ?? ""
    ...
    func makePhoto(){
        ...
        imgVc.delegate = self
        let alert = UIAlertController(title: "select",
                                      message: nil,
        ...
        alert.addAction(UIAlertAction(title: "Test", style: .default, handler: { [weak self] _ in
            guard let self = self,let imgPath = Bundle.main.path(forResource: "photo", ofType: "jpg") else { return }
    ...
    func imagePickerController(_ picker: UIImagePickerController,
        ...
        if let model = BleManager.shared.currentCmdMgr?.outputDeviceModel(){
            ...
            let dt = JLBmpConvert.resize(image!, andResizeTo: size)
            changeImageToBin(dt)
            transpFileLab.text = self.watchBinName
            self.isDial = false
        }
    }
    ...
    func changeImageToBin(_ dt:Data){
        ...
        if model?.sdkType == .type701xWATCH{
            JLBmpConvert.covert(.type701N_ARBG, inFilePath: bmpPath, outFilePath: binPath) { _, _, err in
            }
        } else if model?.sdkType == .type707nWATCH {
            JLBmpConvert.covert(.type707N_ARGB, inFilePath: bmpPath, outFilePath: binPath) { _, _, err in
            }
        } else{
            JLBmpConvert.covert(.type695N_RBG, inFilePath: bmpPath, outFilePath: binPath) { _, _, err in
            }
        }
        self.filePath = binPath
    }
}
