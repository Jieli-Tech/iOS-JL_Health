...
class ContactViewController: BaseViewController {
    ...
    override func initUI() {
        ...
        navigationView.title = "Sync Contacts"
        navigationView.leftBtn.setTitle("Back", for: .normal)
        
        getContactBtn.setTitle("Get Contacts", for: .normal)
        ...
        addContactBtn.setTitle("Add", for: .normal)
        ...
        deleteContactBtn.setTitle("Delete", for: .normal)
        ...
        itemArray.bind(to: subTable.rx.items(cellIdentifier: "ContactCell",cellType: ContactCell.self)) { index, item, cell in
            cell.bind(item)
    ...
    override func initData() {
            ...
            BleManager.shared.currentCmdMgr?.mSmallFileManager
                .cmdSmallFileQueryType(.contacts,result: { fileList in
                    if let file = fileList?.first{
                        ...
                        BleManager.shared.currentCmdMgr?.mSmallFileManager
                            .cmdSmallFileRead(file, result: { op, progress, dt in
                                switch op {
        ...
        addContactBtn.rx.tap.subscribe(onNext: { [weak self] in
            ...
            if self.itemArray.value.count > 5 {
                self.view.makeToast("Only 5 can be added")
                return
            }
        ...
        deleteContactBtn.rx.tap.subscribe(onNext: { [weak self] in
            ...
            if self.itemArray.value.count < 1  {
                self.view.makeToast("At least one contact must be added")
                return
            }
            var items = self.itemArray.value
            items.removeFirst()
    ...
    func pushToDevice(){
        let dt = ContactModel.beData(self.itemArray.value)
        
        guard let fileModel = self.contactsFM else {
            BleManager.shared.currentCmdMgr?.mSmallFileManager
                .cmdSmallFileNew(dt, type: .contacts, result:
                                    { op, progress, st in
                    switch op {
                    case .crcError,.fail,.unknown,.excess,.timeout:
                        self.view.makeToast("Transfer failed",position: .center)
                        break
                    case .doing:
                        self.view.makeToast("Transmitting...",position: .center)
                    case .suceess:
                        self.view.makeToast("Transfer successful",position: .center)
                    default:
                        break
                    }
                })
        ...
        BleManager.shared.currentCmdMgr?.mSmallFileManager
            .cmdSmallFileUpdate(fileModel, data: dt, result: { op, progress in
                switch op {
                case .crcError,.fail,.unknown,.excess,.timeout:
                    self.view.makeToast("Transfer failed",position: .center)
                    break
