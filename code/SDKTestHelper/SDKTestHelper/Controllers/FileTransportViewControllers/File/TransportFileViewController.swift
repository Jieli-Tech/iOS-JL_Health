...
fileprivate enum TransportFileVCType {
    case send
    case receive
}
...
class TransportFileViewController: BaseViewController {
    ...
    override func initData() {
        ...
        navigationView.leftBtn.rx.tap.subscribe { [weak self]_ in
            ...
            if self.isTransporting {
                let alert = UIAlertController(title: "Tips", message: "Transferring, confirm to exit?", preferredStyle: .alert)
        ...
        startBtn.rx.tap.subscribe { [weak self]_ in
            if let path = self?.filePath,path != "" {
                //准备环境
                BleManager.shared.currentEntity?.mCmdManager.mFileManager.cmdPreEnvironment(.bigFileTransmission){status,_,_ in
                    if status == .success {
                        self?.startBtn.setTitleColor(UIColor.gray, for: .normal)
                        self?.startBtn.isUserInteractionEnabled = false
                        
                        //开始传输
                        BleManager.shared.currentEntity?.mCmdManager.mFileManager.cmdBigFileData(path, withFileName: (path as NSString).lastPathComponent, result: { result, progress in
                            switch result {
                            case .transferStart,.transferDownload:
                                ...
                                break
                            case .transferEnd:
                                self?.view.makeToast("Transported successfully",position: .center)
                            ...
                            default:
                                self?.startBtn.setTitleColor(UIColor.black, for: .normal)
                                self?.startBtn.isUserInteractionEnabled = true
                                self?.view.makeToast("Transport error：\(result)",position: .center)
                                self?.deviceFileListView.isUserInteractionEnabled = true
                                self?.isTransporting = false
                            }
                        })
        ...
        deviceFileListView.handleDownloadFile = { [weak self] model in
            ...
            guard let `self` = self else {return}
            if self.isTransporting {
                self.view.makeToast("The transmission is in progress. Repeated initiation is not allowed",position: .center)
                return
            }
            let alert = UIAlertController(title: "Select the way to read back", message: nil, preferredStyle: .actionSheet)
            alert.addAction(UIAlertAction(title: "File Name", style: .default, handler: { _ in
                BleManager.shared.currentEntity?.mCmdManager.mFileManager.cmdFileReadContent(withName: model.fileName, result: { result, sn, data, progressValue in
                    self.handleReadBack(result: result, size: sn, data: data, progressValue: progressValue,model: model)
                })
            }))
            
            alert.addAction(UIAlertAction(title: "File cluster", style: .default, handler: { _ in
                BleManager.shared.currentEntity?.mCmdManager.mFileManager.cmdFileReadContent(withFileClus: model.fileClus, result: { result, sn, data, progressValue in
                    self.handleReadBack(result: result, size: sn, data: data, progressValue: progressValue,model: model)
                })
            }))
            self.present(alert, animated: true)
    ...
    private func cancelTransport(){
        if self.transportType == .send {
            BleManager.shared.currentEntity?.mCmdManager.mFileManager.cmdStopBigFileData()
            self.startBtn.setTitleColor(UIColor.black, for: .normal)
            self.startBtn.isUserInteractionEnabled = true
        }else{
            BleManager.shared.currentEntity?.mCmdManager.mFileManager.cmdFileReadContentCancel()
            self.startBtn.setTitleColor(UIColor.black, for: .normal)
            self.startBtn.isUserInteractionEnabled = true
        }
    }
    ...
    private func handleReadBack(result:JL_FileContentResult,size:UInt32,data:Data?,progressValue:Float,model:JLModel_File){
        ...
        case .reading:
            ...
            self.startBtn.setTitleColor(UIColor.gray, for: .normal)
            self.startBtn.isUserInteractionEnabled = false
            self.transportType = .receive
            self.isTransporting = true
        case .end:
            self.view.makeToast("Read back completed:\(result)")
            self.startBtn.setTitleColor(UIColor.black, for: .normal)
            self.startBtn.isUserInteractionEnabled = true
            self.isTransporting = false
        case .start:
            self.view.makeToast("Reading back start:\(result)")
