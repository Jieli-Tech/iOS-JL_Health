...
class MainViewController: BaseViewController {
    ...
    let itemsArray:[String] = ["File Transport","Default Set","Customer Command","Update"]
    ...
    override func initUI() {
        super.initUI()
        navigationView.title = "JL SDK Test Helper"
        navigationView.rightBtn.isHidden = false
        navigationView.leftBtn.setTitle("Search", for: .normal)
    ...
    @objc func connectStatusChange(_ notification:Notification){
        ...
        if((BleManager.shared.currentEntity) != nil){
            ...
            self.view.makeToast("Initializing...",duration: 5,position: .center)
...
extension MainViewController:UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0,BleManager.shared.currentEntity != nil {
            if indexPath.row == 0{
                cell?.titleLab.text = "Device Name:" + (BleManager.shared.currentEntity?.mPeripheral.name ?? "")
            }
            if indexPath.row == 1{
                cell?.titleLab.text  = "Device EDR addr:" + (BleManager.shared.currentEntity?.mEdr ?? "")
            }
            if indexPath.row == 2{
                cell?.titleLab.text  = "Disconnect"
            }
        }else{
