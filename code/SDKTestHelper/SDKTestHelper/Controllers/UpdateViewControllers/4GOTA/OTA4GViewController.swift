...
class OTA4GViewController: BaseViewController {
    ...
    override func viewDidLoad() {
        ...
        if let cmdMgr = BleManager.shared.currentCmdMgr {
            JLDeviceConfig.share().deviceGet(cmdMgr) { st, sn, model in
    ...
    override func initUI() {
        ...
        selectFileBtn.setTitle("Select 4G OTA file", for: .normal)
        ...
        startOtaBtn.setTitle("Start", for: .normal)
        ...
        stopOtaBtn.setTitle("Stop", for: .normal)
        ...
        statusLab.text = "Not started yet"
    ...
    override func initData() {
        ...
        initializeBtn.rx.tap.subscribe { [weak self]_ in
            ...
            self.manager4G.cmdGetDevice4GInfo(cmdMgr) { st, model in
...
extension OTA4GViewController:JL4GUpgradeDelegate{
    ...
    func jl4GUpgradeResult(_ mgr: JL4GUpgradeManager, status: JL4GUpgradeStatus, progress: Float, code: UInt8, error: Error?) {
        ...
        case .finish:
            if code != 0x00{
                self.statusLab.text = "Update failed"
                self.view.makeToast("\(String(describing: error?.localizedDescription))",position: .center)
            }else{
                self.statusLab.text = "Update success"
            }
            self.startOtaBtn.isUserInteractionEnabled = true
            self.startOtaBtn.alpha = 1
            break
        case .start:
            self.statusLab.text = "Start upgrade"
            break
        case .transporting:
            self.statusLab.text = "Upgrading"
            progressView.progress = progress
            progressLab.text = "\(Int(progress * 100))%"
        case .deviceProcessing:
            self.statusLab.text = "The upgrade data has been sent and the device is being processed."
        @unknown default:
            break
        }
    }
