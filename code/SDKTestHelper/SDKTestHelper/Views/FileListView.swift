...
class FileListView: BaseView {
    ...
    override func initUI() {
        ...
        let model = BleManager.shared.currentEntity?.mCmdManager.mFileManager.getCurrentFileHandleType() ?? .FLASH
        
        titleLab.text = model.beString()+"'s file list"
    ...
    override func initData() {
        ...
        fileListTableView.rx.modelSelected(JLModel_File.self).subscribe { [weak self] item in
            if let model = item.element{
                ...
                if model.fileType == .file{
                    let alert = UIAlertController(title: nil, message: "Do you want to read back this file from the device?\n\(model.fileName)\nThe retrieved file can be viewed in [Select File]", preferredStyle: .alert)
        ...
        refreshBtn.rx.tap.bind { [weak self] in
            let fm = BleManager.shared.currentEntity?.mCmdManager.mFileManager
    ...
    private func createData(){
        ...
        guard let fmanager = BleManager.shared.currentEntity?.mCmdManager else{
            return
        }
        nowModels.removeAll()
