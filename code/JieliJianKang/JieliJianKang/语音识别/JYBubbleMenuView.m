...
//        _buttonsBgView = [[UIImageView alloc] init];
//        _buttonsBgView.image =  [UIImage imageNamed:@"icon_ai_down_bg"];
//        [self addSubview:_buttonsBgView];
        ...
        _buttonsBgView.layer.contents = (__bridge id)[UIImage imageNamed:@"icon_ai_down_bg"].CGImage;
...
-(void)setDirectionPriority:(JYBubbleMenuViewDirectionPriorityType) directionType{
    _directionPriority = directionType;
}
...
- (void)showViewWithButtonModels:(NSArray *)array
    ...
    if (_directionPriority == JYBubbleMenuViewDirectionPriorityType_Down) {
        _buttonsBgView.layer.contents = (__bridge id)[UIImage imageNamed:@"icon_ai_up_bg"].CGImage;
        //Y轴处理，bubble在下面。
        ...
        selfRect.origin.y = selfRect.origin.y-5;
    } else {
        _buttonsBgView.layer.contents = (__bridge id)[UIImage imageNamed:@"icon_ai_down_bg"].CGImage;
        //优先级Up的话，先判断在上面的情况。
...
- (UIView *)drawButtonsWithArray:(NSArray *)array {
    ...
    //确定按钮背景的视图尺寸。
    CGFloat width = rightMargin * 2 + ((2 > countInOneLine)? countInOneLine : 2) * buttonWidth;
    CGFloat height = (2 / countInOneLine + 1) * (2 * topMargin + buttonHeight);
