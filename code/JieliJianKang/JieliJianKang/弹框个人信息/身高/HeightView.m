...
@interface HeightView()<LanguagePtl>{
    float sw;
    float sh;
...
-(void)initUI{
    ...
    if([unitStr isEqualToString:@("英制")]){
        originIndex = 50*0.394;
        totalCount  = 250*0.394;
    }else{
        originIndex = 50;
        totalCount  = 250;
    }
...
-(void)confirmBtnAction:(UIButton *)btn{
    self.hidden = YES;
    if ([_delegate respondsToSelector:@selector(heightAction:)]) {
        [_delegate heightAction:selectValue];
    }
}
...
-(void)setSelectMValue:(NSString *)selectMValue{
    ...
    if([unitStr isEqualToString:@("英制")]){
        unitLabel.text = kJL_TXT("英寸");
    }else{
        unitLabel.text = kJL_TXT("厘米");
    }
