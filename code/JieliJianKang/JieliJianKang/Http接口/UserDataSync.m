            ...
            if (array.count > 0) {
                dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
                    ...
                    [[manager dataTaskWithRequest:req uploadProgress:nil downloadProgress:nil completionHandler:^(NSURLResponse * _Nonnull response, id  _Nullable responseObject, NSError * _Nullable error) {
                        ...
                        if ([code intValue] == 0) {
//                            NSLog(@"uploadHealthData LastHealthHeartRateUpeateTime success");
//                            [[NSUserDefaults standardUserDefaults] setObject:[NSDate date] forKey:[self getUserLastUpeateTimeKeyWithKey:LastHealthHeartRateUpeateTime]];
                        }
                    }] resume];
                });
        ...
        [[JLSqliteManager sharedInstance] checkoutWithStartDate:lastHealthStepCountUpdateTime WithEndDate:[NSDate date] DataByName:tb_step result:^(NSArray * _Nonnull array) {
            if (array.count > 0) {
                dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
                    ...
                    [[manager dataTaskWithRequest:req uploadProgress:nil downloadProgress:nil completionHandler:^(NSURLResponse * _Nonnull response, id  _Nullable responseObject, NSError * _Nullable error) {
                        NSDictionary *dict = responseObject;
//                        NSLog(@"uploadHealthData LastHealthStepCountUpdateTime: %s: dict:%@",__func__,dict);
                        NSNumber *code = dict[@"code"];
                        if ([code intValue] == 0) {
//                            NSLog(@"uploadHealthData LastHealthStepCountUpdateTime success");
//                            [[NSUserDefaults standardUserDefaults] setObject:[NSDate date] forKey:[self getUserLastUpeateTimeKeyWithKey:LastHealthStepCountUpdateTime]];
                        }
                    }] resume];
                });
        ...
        [[JLSqliteManager sharedInstance] checkoutWithStartDate:lastHealthSpoDataUpdateTime WithEndDate:[NSDate date] DataByName:tb_oxyhemoglobin_saturation result:^(NSArray * _Nonnull array) {
            if (array.count > 0) {
                dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
                    ...
                    [[manager dataTaskWithRequest:req uploadProgress:nil downloadProgress:nil completionHandler:^(NSURLResponse * _Nonnull response, id  _Nullable responseObject, NSError * _Nullable error) {
                        NSDictionary *dict = responseObject;
//                        NSLog(@"uploadHealthData LastHealthSpoDataUpdateTime: %s: dict:%@",__func__,dict);
                        NSNumber *code = dict[@"code"];
                        if ([code intValue] == 0) {
//                            NSLog(@"uploadHealthData LastHealthSpoDataUpdateTime success");
//                            [[NSUserDefaults standardUserDefaults] setObject:[NSDate date] forKey:[self getUserLastUpeateTimeKeyWithKey:LastHealthSpoDataUpdateTime]];
                        }
                    }] resume];
                });
        ...
        [[JLSqliteManager sharedInstance] checkoutWithStartDate:lastHealthSleepDataUpdateTime WithEndDate:[NSDate date] DataByName:tb_sleep result:^(NSArray * _Nonnull array) {
            if (array.count > 0) {
                dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
                    ...
                    [[manager dataTaskWithRequest:req uploadProgress:nil downloadProgress:nil completionHandler:^(NSURLResponse * _Nonnull response, id  _Nullable responseObject, NSError * _Nullable error) {
                        NSDictionary *dict = responseObject;
//                        NSLog(@"uploadHealthData LastHealthSleepDataUpdateTime: %s: dict:%@",__func__,dict);
                        NSNumber *code = dict[@"code"];
                        if ([code intValue] == 0) {
//                            NSLog(@"uploadHealthData LastHealthSleepDataUpdateTime success");
//                            [[NSUserDefaults standardUserDefaults] setObject:[NSDate date] forKey:[self getUserLastUpeateTimeKeyWithKey:LastHealthSleepDataUpdateTime]];
                        }
                    }] resume];
                });
        ...
        [JLSqliteWeight s_checkoutWtihStartDate:lastHealthWeightDataUpdateTime withEndDate:[NSDate date] Result:^(NSArray<JL_Chart_Weight *> * _Nonnull charts) {
            if (charts.count > 0) {
                dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
                    ...
                    [[manager dataTaskWithRequest:req uploadProgress:nil downloadProgress:nil completionHandler:^(NSURLResponse * _Nonnull response, id  _Nullable responseObject, NSError * _Nullable error) {
                        NSDictionary *dict = responseObject;
//                        NSLog(@"uploadHealthData LastHealthWeightDataUpdateTime: %s: dict:%@",__func__,dict);
                        NSNumber *code = dict[@"code"];
                        if ([code intValue] == 0) {
//                            NSLog(@"uploadHealthData LastHealthWeightDataUpdateTime success");
//                            [[NSUserDefaults standardUserDefaults] setObject:[NSDate date] forKey:[self getUserLastUpeateTimeKeyWithKey:LastHealthWeightDataUpdateTime]];
                        }
                    }] resume];
                });
...
//            NSLog(@"uploadSportFileWithStartDate: 需要上传的文件数量:%lu", (unsigned long)sportFileArray.count);
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
...
//                        NSLog(@"uploadSportFileWithStartDate: dict:%@", dict);
                        NSNumber *code = dict[@"code"];
                        if ([code intValue] == 0) {
//                            NSLog(@"uploadSportFileWithStartDate success lastdate:%@", [NSDate dateWithTimeIntervalSince1970:sportid]);
...
//    NSLog(@"requestHealthDataBetween: start:%@, end:%@, tbName:%@", start, end, tbName);
    [[manager dataTaskWithRequest:req uploadProgress:nil downloadProgress:nil completionHandler:^(NSURLResponse * _Nonnull response, id  _Nullable responseObject, NSError * _Nullable error) {
//        NSLog(@"requestHealthDataBetween: start:%@, end:%@, tbName:%@, responseObject:%@", start, end, tbName, responseObject);
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
            NSDictionary *dict = responseObject;
...
+ (void)requestSportDataSuccessively {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
        ...
        if (timeInterval > (7 * 24 * 3600)) {
            ...
            NSLog(@"requestSportDataSuccessively1 startDate:%@, endDate:%@", startDate, endDate);
            ...
            NSLog(@"requestSportDataSuccessively2");
            ...
            [UserDataSync requestAndSaveSportDataBetween:startDate End:currentDate BySportType:[NSString stringWithFormat:@"%d", [weakSelf smallFileTypeFromTableName:tb_sport_running]] withBlock:^{
                ...
                NSLog(@"requestSportDataSuccessively finish!");
            }];
        }
    });
}
...
//    NSLog(@"requestAndSaveSportDataBetween1: start:%@, end:%@, type:%@", start, end, type);
    [self requestSportDataBetween:start End:end BySportType:type result:^(NSArray<UserDataSport *> * _Nonnull array) {
        
//        NSLog(@"requestAndSaveSportDataBetween1:%ld", (long)array.count);
//        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
        if (array.count == 0) {
            block();
            return;
...
//                            NSLog(@"下载文件:%@ 成功！finalFilePath:%@", userDataSport.dataPath, finalFilePath);
...
//    NSLog(@"saveSportFileToSportTableWithFilePath, filePath : %@", filePath);
    
    if (fileData == nil || fileData.length < 1) {
        NSLog(@"saveSportFileToSportTableWithFilePath, fileData nothing!");
        return;
    }
...
//    NSLog(@"web_crc : %hu, local_crc : %hu", web_crc, local_crc);
...
//                NSLog(@"gpsFileLength finish!");
            ...
            if (gpsDataType == 0X01) {
                ...
                index += 8;
//                NSLog(@"1=============JLSportLocationTypeStartPacket sport_id:%f, startTime:%u, startTimeInterval:%f, %@", sport_id, (unsigned int)startTime, startTimeInterval, [NSDate dateWithTimeIntervalSince1970:startTimeInterval]);
                ...
                index += 4;
//                NSLog(@"2=============JLSportLocationTypeStartPacket sport_id:%f, longitude:%f, latitude:%f, speed:%f", sport_id, longitude[0], latitude[0], speed);
...
//        NSLog(@"initializeSportDownloadDir %@, error = %@", docFileFolderPath, error);
//        NSLog(@"initializeSportDownloadDir %@, isSuccess = %d", docFileFolderPath, isSuccess);
    }
...
+ (NSString *)initializeSportDir {
    ...
    if (![[NSFileManager defaultManager] fileExistsAtPath:docFileFolderPath]) {
        NSError *error = nil;
        BOOL isSuccess = [[NSFileManager defaultManager] createDirectoryAtPath:docFileFolderPath withIntermediateDirectories:YES attributes:nil error:&error];
//        NSLog(@"initializeSportDir %@, error = %@", docFileFolderPath, error);
//        NSLog(@"initializeSportDir %@, isSuccess = %d", docFileFolderPath, isSuccess);
    }
    return docFileFolderPath;
}
