...
- (instancetype)initWithIndex:(NSInteger)index WithHeight:(float)height
{
    ...
    if (self) {
        ...
        self.frame = CGRectMake(0, 0, index*kTizhong_SCALE*kTizhong_GAP+kTizhongPickGAP, height);
        view_W = index*kTizhong_SCALE*kTizhong_GAP+kTizhongPickGAP;
        view_H = height;
...
-(void)drawLayer:(CALayer *)layer inContext:(CGContextRef)ctx{
    /*--- 描绘刻度 ---*/
    for (NSInteger i = 0; i <= mIndex*kTizhong_SCALE; i++)
    {
        /*--- 小刻度 ---*/
        if (i%10 != 0) {
            [self drawScale10:i*kTizhong_GAP+kTizhongPickGAP/2.0 Context:ctx];
        }
        
        /*--- 大刻度 ---*/
        if (i%10 == 0) {
            [self drawScale20:i*kTizhong_GAP+kTizhongPickGAP/2.0 Context:ctx];
            
            /*--- 刻度数字 ---*/
            long fp = (i+_startPiont)/10.0;
            NSString *text = [NSString stringWithFormat:@"%ld",fp];
            [self drawLabel:i*kTizhong_GAP+kTizhongPickGAP/2.0 Text:text Layer:layer];
        }
    }
}
