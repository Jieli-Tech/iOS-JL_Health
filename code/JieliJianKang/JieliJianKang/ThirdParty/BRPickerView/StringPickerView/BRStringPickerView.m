...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(nullable UIView *)view {
    ...
    if (self.pickerMode == BRStringPickerComponentSingle) {
        if(self.type == 0){ //心率
            label.frame = CGRectMake(0, 0, self.pickerView.frame.size.width-100, self.pickerStyle.rowHeight);
        }else{
            label.frame = CGRectMake(0, 0, self.pickerView.frame.size.width-20, self.pickerStyle.rowHeight);
        }
