...
- (instancetype)initWithFrame:(CGRect)frame {
    if ([super initWithFrame:frame]) {
        ...
        self.barWidth = 14;
        self.barNormalColor = PGColor(163, 134, 249);
...
- (CGFloat)setupLineAndVerticalLabel {
    ...
    float heightGap = (self.frame.size.height - maxHeight - VerticalLineGap) / 7.0;
    
    float highestValueEColumnChart = [self maxValueForCharDataModels].value * 1.1;
    float valueGap = highestValueEColumnChart / 7.0;
    
    NSMutableArray *array = [NSMutableArray array];
    for (int i = 0; i < 7; i++) {
        NSString *text = [[NSString stringWithFormat:@"%.f", valueGap * (7 - i)] stringByAppendingString:[self maxValueForCharDataModels].unit];
        ...
        eColumnChartLabel.text = [[NSString stringWithFormat:@"%.f", valueGap * (7 - i)] stringByAppendingString:[self maxValueForCharDataModels].unit];
...
- (void)logic {
    CGFloat verticalLinePoX = 10;//[self setupLineAndVerticalLabel];
    NSInteger totalCount = [self.dataSource charDataModels].count;
    float realMaxValue = [self maxValueForCharDataModels].value;
    float maxValue = realMaxValue * 1.3;
...
//    self.verticalLine = verticalLine;
    float heightGap = bottomLine.frame.origin.y / 7.0;
    for (int i = 0; i < 7; i++) {
    ...
    CGFloat barHeight = self.frame.size.height - 3 - maxHeight - VerticalLineGap;
    for (int i = 0; i < totalCount; i++) {
        ...
        PGBar *bar = [[PGBar alloc] initWithFrame:CGRectMake(barPosX, 3, self.barWidth, barHeight)];
...
//            NSLog(@"y==========self.goalNum :%d, realMaxValue:%f, bottomLineY:%f, barHeight:%f, y:%f", self.goalNum, realMaxValue, bottomLineY, barHeight, y);
        ...
        barChartLabel.fontColor = self.bottomLabelFontColor;
        [barChartLabel setTextAlignment:NSTextAlignmentCenter];
        barChartLabel.text = self.goalNumText;
        [self addSubview:goalLine];
        [self addSubview:barChartLabel];
    }
}
