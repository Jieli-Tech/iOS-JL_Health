...
-(void)addTask:(SyncObject *)objc{
    if (objc.smallFileManager == nil) {
        NSLog(@"同步文件的句柄为空，请检查是否已连接设备！");
        return;
    }
    NSLog(@"SyncObject:%d",objc.type);
    if (requestArray.count == 0) {
        [requestArray addObject:objc];
        dispatch_semaphore_signal(semaphore);
        NSLog(@"contiune sync thread %d",__LINE__);
    }else{
        [requestArray addObject:objc];
    }
...
-(void)requireToDevice{
    while (1) {
        if (requestArray.count!=0){
            ...
            NSLog(@"output:%d",requestArray.firstObject.type);
        }
        NSLog(@"pause sync thread");
                ...
                [objc.smallFileManager cmdSmallFileQueryType:objc.type Result:^(NSArray<JLModel_SmallFile *> * _Nullable array) {
                    ...
                    if (array.count != 0) {
                        ...
                        if([self->_delegate respondsToSelector:@selector(syncTaskFinish:type:)]){
                            ...
                            NSLog(@"不需要更新文件:%s,%d",__func__,__LINE__);
                        }
                        if (self->requestArray.count>0) {
                            [self->requestArray removeObjectAtIndex:0];
                        }
                        dispatch_semaphore_signal(self->semaphore);
                        NSLog(@"contiune thread %d",__LINE__);
                    }
                }];
            });
        }break;
...
//-(void)startSyncForAllBy:(SyncObject *)objc FileList:(NSArray<JLModel_SmallFile *> *)filelist{
//    NSMutableArray *newFileList = [NSMutableArray arrayWithArray:filelist];
//    JLModel_SmallFile  *reqSf = newFileList.lastObject;
...
//                    NSLog(@"收到完整文件:%s,%d",__func__,__LINE__);
//                }
//            });
//            [newFileList removeLastObject];
//            if ( newFileList.count>0){
//                [self startSyncForAllBy:objc FileList:newFileList];
//            }else{
//                if([self->_delegate respondsToSelector:@selector(syncTaskFinish:type:)]){
//                    [self->_delegate syncTaskFinish:NULL type:objc.type];
//                    NSLog(@"不需要更新文件:%s,%d",__func__,__LINE__);
            ...
            if (targetSf) {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    if([self->_delegate respondsToSelector:@selector(syncTaskFinish:type:)]){
                        ...
                        NSLog(@"未找到对应运动ID:%s,%d",__func__,__LINE__);
                    }
                });
            }
        }];
