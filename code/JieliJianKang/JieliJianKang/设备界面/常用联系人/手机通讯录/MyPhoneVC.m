            ...
            [contactManager fetchAllPerson:^(NSArray<JHPersonModel *> *allPersons) {
                ...
                for (JHPersonModel *personModel in allPersons) {
                    ...
                    for (JHPersonModel *originalPersonModel in self.originalContactArray) {
                        if ([personModel.phoneNum isEqualToString:originalPersonModel.phoneNum]) {
                            addToLocal = false;
                        }
                        if(originalPersonModel.fullName){
                            if ([personModel.fullName hasPrefix:originalPersonModel.fullName]) {
                                addToLocal = false;
                            }
                        }else{
                            addToLocal = true;
                    ...
                    if (addToLocal) {
                        [finalPersons addObject:personModel];
                    }
                }
        ...
        if ([JLFileTransferHelper getContactTargetDev] == JL_CardTypeFLASH2) {
            [JLFileTransferHelper sendContactFileToFlashWithFileName:@"CALL.TXT"
                ...
                if (type == JLFileTransferOperateTypeSuccess) {
                    ...
                    if ([self.delegate respondsToSelector:@selector(transferCallFileFinished)]) {
                        [self.delegate transferCallFileFinished];
                    }
                }
            }];
        } else {
