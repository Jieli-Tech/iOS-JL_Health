...
#import "SleepHourMinutePickerView.h"

@interface SleepDetectionVC ()<SleepHourMinutePickerDelegate,JL_WatchProtocol>{
    ...
    SleepHourMinutePickerView *sleepHourMinutePickerViewStartTime;
    SleepHourMinutePickerView *sleepHourMinutePickerViewEndTime;
...
@implementation SleepDetectionVC
...
-(void)requireDataFromDevice{
    ...
    [w w_InquireDeviceFuncWith:JL_WATCH_SETTING_SLEEP_DETECTION withEntity:kJL_BLE_EntityM];
}
...
-(void)initUI{
    ...
    fullDayLabel.frame = CGRectMake(16,19,sw-16,21);
    fullDayLabel.numberOfLines = 0;
    [contentView2 addSubview:fullDayLabel];
    fullDayLabel.text = kJL_TXT("全天开启");
    fullDayLabel.font = [UIFont fontWithName:@"PingFangSC" size: 15];
    fullDayLabel.textColor = kDF_RGBA(36, 36, 36, 1.0);
    fullDayLabel.textAlignment = NSTextAlignmentLeft;
    ...
    sleepHourMinutePickerViewStartTime = [[SleepHourMinutePickerView alloc] initWithFrame:CGRectMake(0, 0, sw, sh)];
    [self.view addSubview:sleepHourMinutePickerViewStartTime];
    sleepHourMinutePickerViewStartTime.delegate = self;
    sleepHourMinutePickerViewStartTime.hidden = YES;
    sleepHourMinutePickerViewStartTime.type = 0;
    
    sleepHourMinutePickerViewEndTime = [[SleepHourMinutePickerView alloc] initWithFrame:CGRectMake(0, 0, sw, sh)];
    [self.view addSubview:sleepHourMinutePickerViewEndTime];
    sleepHourMinutePickerViewEndTime.delegate = self;
    sleepHourMinutePickerViewEndTime.hidden = YES;
    sleepHourMinutePickerViewEndTime.type = 1;
}
...
-(void)startTimeClick{
    sleepHourMinutePickerViewStartTime.hidden = NO;
}

-(void)endTimeClick{
    sleepHourMinutePickerViewEndTime.hidden = NO;
}
...
-(void)SleepHourMinutePickerStartTime:(NSMutableArray *) selectArray{
    ...
    sleepHourMinutePickerViewEndTime.endTimeselectIndexs = @[@(endHour),@(endMin)];
...
-(void)SleepHourMinutePickerEndTime:(NSMutableArray *) selectArray{
    ...
    sleepHourMinutePickerViewStartTime.startTimeselectIndexs = @[@(startHour),@(startMin)];
...
-(void)sendDataToDevice{
    ...
    JLSleepMonitorModel *model1 = [[JLSleepMonitorModel alloc] initWithSwitchStatus:currentType Begin:startTimer End:endTimer];
