...
-(void)viewWillAppear:(BOOL)animated{
    if (self.rtcmodel.ringInfo) {
        ...
        JLModel_Device *device = [kJL_BLE_CmdManager outputDeviceModel];
...
//          self.rtcmodel.rtcHour,
//          self.rtcmodel.rtcMin,
//          self.rtcmodel.rtcSec,
//          self.rtcmodel.rtcEnable,
//          self.rtcmodel.rtcMode,
//          self.rtcmodel.rtcIndex,
//          self.rtcmodel.rtcName);
//
//    JLModel_RTC *rtc1 = [JLModel_RTC new];
//    rtc1.rtcYear = self.rtcmodel.rtcYear;
//    rtc1.rtcMonth = self.rtcmodel.rtcMonth;
//    rtc1.rtcDay = self.rtcmodel.rtcDay;
//    rtc1.rtcHour = self.rtcmodel.rtcHour;
//    rtc1.rtcMin = self.rtcmodel.rtcMin;
//    rtc1.rtcSec = self.rtcmodel.rtcSec;
//    rtc1.rtcEnable = self.rtcmodel.rtcEnable;
//    rtc1.rtcMode = self.rtcmodel.rtcMode;
//    rtc1.rtcIndex = self.rtcmodel.rtcIndex+1;
//    rtc1.rtcName = self.rtcmodel.rtcName;
//    rtc1.ringInfo = self.rtcmodel.ringInfo;
//    rtc1.RingData = self.rtcmodel.RingData;
//
//    NSArray *arr = @[self.rtcmodel,rtc1];
    
    
    [kJL_BLE_CmdManager.mAlarmClockManager cmdRtcSetArray:@[self.rtcmodel]
...
- (void)deleteBtnAction:(UIButton *)btn {
    ...
    [kJL_BLE_CmdManager.mAlarmClockManager cmdRtcDeleteIndexArray:@[@(self.rtcmodel.rtcIndex)]
                                                           Result:^(JL_CMDStatus status, uint8_t sn,
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(device.rtcAlarmType == JL_RTCAlarmTypeYES){
        if(mAlarmSetting.isTime==1 && mAlarmSetting.isInterval==1 && mAlarmSetting.isCount==1){ //响铃时长、再响间隔时间、再响响铃次数
            if (indexPath.row == 0) {
                cell.typeImg.image = [UIImage imageNamed:@"clock_icon_name"];
            }
            if (indexPath.row == 1) {
                cell.typeImg.image = [UIImage imageNamed:@"clock_icon_bell"];
            }
            if (indexPath.row == 2) {
                cell.typeImg.image = [UIImage imageNamed:@"clock_time_nol"];
            }
            if (indexPath.row == 3) {
                cell.typeImg.image = [UIImage imageNamed:@"clock_interval_nol"];
            }
        }
        if(mAlarmSetting.isTime==1 && mAlarmSetting.isInterval==1 && mAlarmSetting.isCount==0){
            if (indexPath.row == 0) {
        ...
        if(mAlarmSetting.isTime==1 && mAlarmSetting.isInterval==1 && mAlarmSetting.isCount==1){ //响铃时长、再响间隔时间、再响响铃次数
            switch (indexPath.row) {
                case 0:{
                    ...
                    vc.delegate = self;
                    vc.txfdStr = nameArray[indexPath.row];
