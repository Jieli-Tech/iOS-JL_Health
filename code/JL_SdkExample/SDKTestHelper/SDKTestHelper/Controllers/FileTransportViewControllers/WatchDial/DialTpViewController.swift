...
class DialTpViewController: BaseViewController, UIImagePickerControllerDelegate & UINavigationControllerDelegate {
    ...
    func imagePickerController(_ picker: UIImagePickerController, 
        ...
        if let model = BleManager.shared.currentCmdMgr?.outputDeviceModel(){
            ...
            let dt = BitmapTool.resize(image!, andResizeTo: size)
            changeImageToBin(dt)
    ...
    func changeImageToBin(_ dt:Data){
        ...
        guard let image = UIImage(data: dt) else{
            ECPrintError("Image data format error", self, "\(#function)", #line)
            return
        }
        let width = image.size.width
        let height = image.size.height
        let size = CGSize(width: width, height: height)
        let bitMap = BitmapTool.convert_B_G_R_A_Bytes(from: image)
        ...
        if model?.sdkType == .type701xWATCH{
            OCHelper.handleBr28Bmp(bmpPath, size: size, binPath: binPath)
        }else{
            OCHelper.handleBr23mp(bmpPath, size: size, binPath: binPath)
        }
