...
class OTAViewController: BaseViewController {
    ...
    func handleOtaResult(_ status:JL_OTAResult,_ progress:Float){
        switch status {
        case .success:
            self.view.makeToast(R.localStr.otaSuccess(),position: .center)
        ...
        case .upgrading:
            self.statusLab.text = R.localStr.upgradingPhase2()
        ...
        case .reconnect:
            ...
            self.view.makeToast(R.localStr.reconnect(),position: .center)
            break
        case .reboot:
            self.view.makeToast(R.localStr.restarting(),position: .center)
            self.statusLab.text = R.localStr.restarting()
        ...
        case .preparing:
            self.statusLab.text = R.localStr.upgradingPhase1()
            self.progressLab.text = "\(Int(progress * 100))%"
            self.progressView.progress = progress
        case .prepared:
            self.statusLab.text = R.localStr.readyToComplete()
            self.view.makeToast(R.localStr.readyToComplete(),position: .center)
