...
class PackageResViewController: BaseViewController {
    ...
    override func initUI() {
        ...
        if self.items.value.count == 0{
            self.view.makeToast(R.localStr.pcm2WTSRecordFinish(),duration: 3,position: .center)
        }
...
fileprivate class PackageCell:UITableViewCell{
    ...
    func makeModel(_ md:TipsVoiceModel){
        self.mainLab.text = "\(R.localStr.fileName()):"+md.file
        self.subLab.text = "\(R.localStr.nickName()):"+md.nickName
        self.model = md
    }
