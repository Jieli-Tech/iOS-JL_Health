...
class AlarmViewController: BaseViewController {
    ...
    override func initUI() {
        ...
        itemsArray.bind(to: subTable.rx.items(cellIdentifier: "AlarmCell", cellType: AlarmCell.self)) { [weak self](row, element, cell) in
            guard let `self` = self else {return}
            cell.bind(element)
            cell.isOnBlock = {
                BleManager.shared.currentCmdMgr?.mAlarmClockManager.cmdRtcSetArray([cell.alarmModel!], result: { status, opSN, dt in
                    if status == .success{
    ...
    override func initData() {
        ...
        addButton.rx.tap.subscribe { [weak self](_) in
            ...
            let rtc:JLModel_RTC = JLModel_RTC()
        ...
        subTable.rx.modelDeleted(JLModel_RTC.self).subscribe { [weak self](item) in
            guard let `self` = self else {return}
            BleManager.shared.currentCmdMgr?.mAlarmClockManager.cmdRtcDeleteIndexArray([NSNumber(value: item.rtcIndex)], result: { status, sn, data in
                if status == .success{
