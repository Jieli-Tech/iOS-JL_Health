...
class AlarmDetailViewController: BaseViewController {
    ...
    override func initUI() {
        ...
        itemsArray.bind(to: subCollect!.rx.items(cellIdentifier: "AlarmDateCollectionCell", cellType: AlarmDateCollectionCell.self)) { (row, element, cell) in
            cell.bind(element)
    ...
    override func initData() {
        ...
        alarmNameButton.rx.tap.subscribe { [weak self](_) in
            ...
            alertView.addAction(UIAlertAction(title: R.localStr.confirm(), style: .default, handler: { [weak self](action) in
                guard let self = self else { return }
        ...
        ringNameButton.rx.tap.subscribe { [weak self](_) in
            ...
            alertSheel.addAction(UIAlertAction(title: R.localStr.defaultRings(), style: .default, handler: { action in
                self.dfSelectView.isHidden = false
            }))
            alertSheel.addAction(UIAlertAction(title: R.localStr.deviceRings(), style: .default, handler: { action in
            ...
            alertSheel.addAction(UIAlertAction(title: R.localStr.cancel(), style: .cancel, handler: { action in
            }))
        ...
        confirmButton.rx.tap.subscribe { [weak self](_) in
            ...
            BleManager.shared.currentCmdMgr?.mAlarmClockManager.cmdRtcSetArray([model], result: { status, opSN, dt in
                if status == .success{
